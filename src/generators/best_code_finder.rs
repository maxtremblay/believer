...
impl<'a, G: CodeGenerator> BestCodeFinderFromErasure<'a, G> {
    ...
    pub fn find_best_code_simulating_until_n_events_are_found_with_rng<R: Rng>(
        &self,
        n_events: usize,
        rng: &mut R,
    ) -> (Option<ParityCheckMatrix>, SimulationResult) {
        NEventsBestCodeFinderFromErasure::from(self)
...
struct NEventsBestCodeFinderFromErasure<'a, G: CodeGenerator> {
    code_finder: &'a BestCodeFinderFromErasure<'a, G>,
    n_events: usize,
    random_seeds: Vec<u64>,
}

impl<'a, G: CodeGenerator> NEventsBestCodeFinderFromErasure<'a, G> {
    fn from(code_finder: &'a BestCodeFinderFromErasure<'a, G>) -> Self {
        Self { code_finder, n_events: 0, random_seeds: Vec::new() }
    }
...
mod test {
    ...
    #[test]
    fn reproductibility_for_finding_best_ldpc_code_simulating_until_n_events_are_found() {
        let rng = ChaCha8Rng::seed_from_u64(123);
        let generator = RegularLDPCCodeGenerator::new(3, 4, 2, 4);
        ...
        let code_and_result_0 = code_finder
            .find_best_code_simulating_until_n_events_are_found_with_rng(50, &mut rng.clone());
