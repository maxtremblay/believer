...
impl<R: Rng> CodeConstructor<R> {
    ...
    fn generate_active_layer(&mut self) {
        ...
        blocks.into_iter().for_each(|block| self.generate_checks_over(block));
    }
    ...
    fn get_blocks_for_active_layer(&self) -> Blocks {
        ...
        Blocks { n_blocks, block_length, active_block: 0 }
    }

    fn get_n_blocks_for_active_layer(&self) -> usize {
        self.n_blocks_per_layer.pow(self.n_layers - self.active_layer)
    }
