...
mod test {
    ...
    fn doesnt_include_same_bit_twice() {
        ...
        let first_check = generator.over_bits(vec![0, 1]).generate(2);
    ...
    fn doesnt_create_cycle_smaller_than_minimal_girth() {
        ...
        let first_check = generator.over_bits(vec![0, 1, 2]).generate(3);
        assert_eq!(first_check, Some(vec![0, 1, 2]));

        let second_check = generator.over_bits(vec![2, 3]).generate(2);
        assert_eq!(second_check, Some(vec![2, 3]));

        // A check over [0, 3] will create a 6-cycle.
        let third_check = generator.over_bits(vec![0, 3]).generate(2);
        assert_eq!(third_check, None);

        // Possible checks are [0, 4] or [3, 4]
        let fourth_check = generator.over_bits(vec![0, 3, 4]).generate(2);
