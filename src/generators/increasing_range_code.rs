...
impl IncreasingRangeCodeGenerator {
    ...
    fn get_random_range<R: Rng + ?Sized>(&self, range_length: usize, rng: &mut R) -> Vec<usize> {
    ...
    fn generate<R: Rng + ?Sized>(&self, rng: &mut R) -> ParityCheckMatrix {
        let mut check_generator =
            RandomCheckGenerator::new()
                .with_max_bit_degrees(self.max_bit_degree)
        ...
        for c in 0..self.n_checks {
            check_generator.over_bits(self.get_random_range(range_length, rng));
            if let Some(check) = check_generator.generate(self.max_check_degree, rng) {
                checks.push(check)
            }
